...
def main():
    ...
    tbl.to_csv(outdir / "swpc_match_summary_all.csv", index=False)
    print(f"[match] summary (all combos) saved → "
          f"{outdir/'swpc_match_summary_all.csv'}  ({len(tbl)} rows)")
    ...
    fin.to_csv(outdir / f"swpc_match_{combo}.csv", index=False)
    print(f"[match] FINAL combo ({combo}) saved → "
          f"{outdir/('swpc_match_'+combo+'.csv')}")
