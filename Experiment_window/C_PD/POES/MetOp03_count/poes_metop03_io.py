"""
poes_metop03_io.py  (v2 — 채널 1개 = 파일 1개, MultiIndex 반환)
==========
...
def save_parquet(df: pd.DataFrame, meta: dict,
    ...
    print(f'[poes_metop03_io] Parquet 저장: {directory}', flush=True)
...
def load_parquet(directory: Union[str, Path],
    ...
    _ensure_pyarrow()
    directory = Path(directory)
    print(f'[poes_metop03_io] Parquet 로드: {directory}', flush=True)
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 2:
        print('사용법: python poes_metop03_io.py <parquet_dir>')
        sys.exit(0)
    d, m = load(sys.argv[1])
    summary(d, m)