...
사용법:
  # JSON
  python poes_noaa19_build_cache.py --root ./poes_nc --start 201901 --end 202512 --out poes_n19_cache.json

  # Parquet
  python poes_noaa19_build_cache.py --root ./poes_nc --start 201901 --end 202512 --out poes_n19_cache_parquet

  # 둘 다 + 1분 리샘플
  python poes_noaa19_build_cache.py --root ./poes_nc --start 201901 --end 202512 \\
      --out poes_n19_cache.json --also-parquet poes_n19_cache_parquet --resample 1min

옵션:
  --root          nc 파일 루트 폴더 (poes_noaa19_download.py 의 --out 경로; 연도 하위폴더 구조)
  --start         시작 월 YYYYMM
  --end           종료 월 YYYYMM
  --out           출력 경로 (.json -> JSON, 확장자없음 -> Parquet)
  --also-parquet  JSON 외에 Parquet 도 저장할 디렉터리
  --resample      리샘플 주기 (기본 '1min'; 'none' 이면 원본 2초 유지)
  --sat-code      파일명 코드 (기본 n19)
...
def build_cache(root: Path, sat_code: str, start_ym: str, end_ym: str,
    ...
    for sensor in parts:
        ...
        for c in combined.columns:
            if c in GEO_CHANNELS:
                continue
            tpl = poes_noaa19_io.flat_to_tuple(c)
            if tpl is None:
                print(f'  [WARN] 채널명 파싱 실패, 건너뜀: {c}')
                continue
            series_dict[tpl] = combined[c]
            stem = poes_noaa19_io.tuple_to_fname(tpl)
    ...
    if out_path.suffix:
        print('[WARN] 이 빌더는 parquet 디렉터리 출력만 지원합니다(.json 미지원). '
              '확장자 없는 경로를 주세요.')
    poes_noaa19_io.save_parquet(count_df, global_meta, out_path, geo=geo_df)
    if also_parquet:
        poes_noaa19_io.save_parquet(count_df, global_meta, also_parquet, geo=geo_df)
...
def main():
    ...
    parser.add_argument('--sat-code', default='n19', help='파일명 위성 코드 (기본 n19)')
