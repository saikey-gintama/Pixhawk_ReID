...
KSEM swpc_alert_espe_match.py 의 POES 이식판:
...
출력 (outdir = <--out>/<runtag>/):
  swpc_match_summary_all_<runtag>.csv   전 (channel×k×onset×peak) 조합 POD/FAR
  swpc_match_<runtag>.csv               FINAL 조합(--k/--onset/--peak) 채널별, POD 내림차순
...
사용 (POES 루트에서):
  python swpc_alert_espe_match_poes.py \
    ...
    --catalog ../SWPC_Alert/swpc_espe_cache_parquet \
