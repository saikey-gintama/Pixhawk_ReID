"""
swpc_alert_espe_blc1_match.py
=============================
BL-C1(고정임계) 검출 이벤트 CSV ↔ SWPC ALTEF3 전자 경보 카탈로그 매칭 + 시각화.

swpc_alert_espe_match.py 의 BL-C1 전용 복제본. 매칭 로직(match_events),
...
범용 평가기 성격은 그대로: swpc_alert_espe_io 만 import, CSV 경로를 인자로 받음.
...
def main():
    ap = argparse.ArgumentParser(description="BL-C1 검출 이벤트 ↔ SWPC eSPE 매칭/시각화")
    ...
    summary_path = outdir / f"swpc_match_summary_all_{name}.csv"
    final_path   = outdir / f"swpc_match_{name}.csv"
