...
========================
검출 이벤트 CSV ↔ SWPC ALTEF3 전자 경보 카탈로그 매칭 + 시각화.

noaa_goes_spe_match.py 와 동일한 인터페이스/출력 규칙.
차이: 카탈로그가 SWPC ESPE (>2 MeV 전자, >1000 pfu) 이라는 점만.
...
사용:
  ...
  python swpc_alert_espe_match.py \
    --events fsm_onset_blc1_lowe_...csv --catalog . [--out swpc_match_output]
...
# _match_core 는 ../NOAA_GOES/ 에 위치
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "NOAA_GOES"))
import swpc_alert_espe_io as spe_io
...
def run_one(events_csv, args, cat):
    ...
    print(f"[match] summary saved -> swpc_match_summary_all_{name}.csv ({len(tbl)} rows)")

    fin = _final_table(tbl, no_peak, args)
    fin.to_csv(outdir / f"swpc_match_{name}.csv", index=False)
    print(f"[match] FINAL saved -> swpc_match_{name}.csv")
    print(fin.to_string(index=False))

    plot_pod_far_scatter(fin, f"SWPC match  {name}",
...
def main():
    ...
    ap.add_argument("--catalog", default=".", help="SWPC 캐시 경로(parquet 디렉터리 or json)")
    ap.add_argument("--tol", type=float, default=MATCH_TOL_H, help="매칭 허용오차(h)")
    ap.add_argument("--out", default="swpc_match_output", help="출력 루트 폴더")
