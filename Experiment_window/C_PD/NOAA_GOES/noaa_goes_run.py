...
NOAA(양성자 SPE 카탈로그)에 일괄 매칭한다.
noaa_goes_spe_match.py 가 있는 폴더에서 실행.

파일명에서 k / onset / peak 자동 추출 후 매칭 스크립트에 전달.
  fsm_event_quietoff_mad_w30_k10_on0.5_pk2.csv -> --k 10 --onset 0.5 --peak 2

사용:
  python noaa_goes_run.py          # 실행
  python noaa_goes_run.py --dry    # 명령만 출력 (실행 안 함)
"""
import argparse
import re
import subprocess
...
MATCH_SCRIPT   = Path("./noaa_goes_spe_match.py")
CATALOG        = Path("./noaa_goes_spe_cache_parquet")
LOG_TAG        = "noaa"
# ─────────────────────────────────────────────────────────────────
