"""
ana1_channel_stats_poes.py  (축 1 — 채널별 원본 통계)
====================================================
POES count 캐시(채널 1개 = 파일 1개, MultiIndex io)에서 각 채널의 *원본 그 자체*
통계를 낸다. 카탈로그·geo 비의존 — count 파일만 사용. 전 기간 통으로 집계.
...
사용:
  python ana1_channel_stats_poes.py --io poes_metop03_io --cache <parquet_dir> --out <out_dir>
  (io 모듈명을 인자로 받아 metop03/noaa19 동일 코드로 처리)
"""
...
def _import_io(io_arg: str, cache_dir: str):
    """io 모듈 임포트. io_arg 는 모듈명(poes_metop03_io). 캐시 폴더와 그 부모를
    sys.path 에 넣어, POES 루트에서 실행해도 위성 폴더 안의 io 를 찾게 한다.
    경로(슬래시 포함)를 줘도 stem 만 모듈명으로 사용."""
...
def main():
    p = argparse.ArgumentParser(description="축1 채널별 원본 통계 (POES)")
    p.add_argument("--io", required=True, help="io 모듈명 (poes_metop03_io | poes_noaa19_io)")
    p.add_argument("--cache", required=True, help="parquet 캐시 디렉터리")
    ...
    for col in df.columns:
        name = io.tuple_to_fname(tuple(col))   # 파일명 stem 을 채널 라벨로
        s = df[col]
        st = channel_stats(s)
        st = {"channel": name, "species": col[0], "direction": col[1],
              "energy": col[2], **st}
        rows.append(st)
